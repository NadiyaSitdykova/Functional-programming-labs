...
import Prelude (Show,Read,error,otherwise)
  
---------------------------------------------
-- Синтаксис лямбда-выражений

-- Эквивалентные определенияexample1 x  = x
example1'   = \x -> x
...
natDivMod n m = if' (natLt n m) 
		(Pair Zero n) 
		(Pair (Succ $ natDiv (n -. m) m) (natMod (n -. m) m))
...
gcd :: Nat -> Nat -> Nat
gcd Zero Zero = error "GCD of Zero and Zero is Infinity"
gcd n Zero = n
...
ratInv :: Rat -> Rat
ratInv (Rat (Pos Zero) _) = error "There is no inverse for Zero"
